...
document.addEventListener('DOMContentLoaded', function() {
    ...
    enhanceWorkSection();
    initInteractiveBallGame(); // Ahora solo afectará a la sección de contacto
    setupContactSection();
});
...
/**
 * Inicializa el juego interactivo con una bola grande que empuja cubos pequeños
 * SOLO en la sección de contacto
 */
function initInteractiveBallGame() {
    // Solo seleccionar la sección de contacto
    const contactSection = document.getElementById('contact');
    if (!contactSection) return;
    
    // Crear contenedor de juego si no existe
    let gameContainer = contactSection.querySelector('.ball-game-container');
    
    if (!gameContainer) {
        gameContainer = document.createElement('div');
        gameContainer.className = 'ball-game-container';
        gameContainer.id = 'game-container-contact';
        
        // Añadir contenedor al inicio de la sección
        contactSection.insertBefore(gameContainer, contactSection.firstChild);
    }
    ...
    // Inicializar el juego sólo para la sección de contacto
    initSectionGame('contact');
...
socialStyles.textContent = `
    ...
    .instagram-icon .social-icon {
        ...
        background-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white"><path d="M12 0C8.74 0 8.333.015 7.053.072 5.775.132 4.905.333 4.14.63c-.789.306-1.459.717-2.126 1.384S.935 3.35.63 4.14C.333 4.905.131 5.775.072 7.053.012 8.333 0 8.74 0 12s.015 3.667.072 4.947c.06 1.277.261 2.148.558 2.913.306.788.717 1.459 1.384 2.126.667.666 1.336 1.079 2.126 1.384.766.296 1.636.499 2.913.558C8.333 23.988 8.74 24 12 24s3.667-.015 4.947-.072c1.277-.06 2.148-.262
